...
- **市场环境上下文（技术面之外的补充分析）**：自动抓取并注入
  涨跌家数与成交额排名、涨停/跌停池及连板高度、所属行业板块行情与近5日趋势、
  北向资金摘要、龙虎榜上榜统计、个股最新新闻，以及尽力而为的网页检索
  （可选安装 `ddgs` 启用 DuckDuckGo，失败自动退回 Bing 中文站）；
  预置技能要求 AI 基于这些数据输出五维结论：
  市场情绪 / 板块效应 / 消息面 / 机构抱团与龙头属性 / 短线炒作预期，
  每项必须给出明确判断+量化依据，数据缺口如实声明而非编造
- **联网搜索（DeepSeek 式）**：侧边栏开启后，AI 自主判断何时需要外部信息，
  自动选择关键词发起**多轮** `web_search` / `read_page` 工具调用并筛选结果，
  回答末尾自动列出编号参考来源；OpenRouter 厂商额外支持 `:online` 原生联网快捷模式
...
├── report.py         # 综合报告组装
├── market_context.py # 市场环境快照(情绪/板块/龙虎榜/北向/新闻/网页检索)
...
- 市场环境数据依赖 akshare 东财系接口，接口字段变动可能导致个别维度降级为"获取失败"
- 网页检索为尽力而为：默认无 Key 方案（DuckDuckGo/Bing），受网络环境影响可能不可用；
  北向资金披露规则调整时该维度数据可能滞后或缺席
